...
import { 
  ...
  RefreshCw, Layers, Upload, Eye, Download, Trash2, History, LogOut, CreditCard, ArrowLeft
} from 'lucide-react';
...
export const StudentDetailView: React.FC<StudentDetailViewProps> = ({ 
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.15),transparent_24%),radial-gradient(circle_at_top_right,rgba(14,165,233,0.12),transparent_22%),radial-gradient(circle_at_bottom,rgba(168,85,247,0.10),transparent_26%),linear-gradient(135deg,#f8fbff_0%,#eef4ff_42%,#f7f3ff_100%)] px-4 py-6 sm:px-6 lg:px-8 fade-in pb-16">
      <div className="mx-auto max-w-7xl space-y-6">
        
      {/* 1. Breadcrumb navigation */}
      <div className="silver-gradient-panel flex flex-col items-start justify-between gap-4 rounded-[28px] p-5 sm:flex-row sm:items-center">
        <div className="space-y-1">
          <button
            type="button"
            onClick={onClose}
            className="inline-flex items-center gap-2 rounded-xl border border-[var(--color-accent-orange)] bg-[var(--color-accent-orange)] px-3.5 py-2 text-[11px] font-black uppercase tracking-[0.18em] text-white shadow-lg shadow-[rgba(200,146,44,0.28)] transition hover:scale-[1.01] hover:bg-[var(--color-accent-orange-hover)]"
          >
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs text-text-muted font-medium">
            <button onClick={onClose} className="hover:text-accent-orange flex items-center gap-1 transition-all font-bold">
              Dashboard
            </button>
            <span className="text-[10px]">/</span>
            <button onClick={onClose} className="hover:text-accent-orange transition-all font-bold">
              {breadcrumbSource}
            </button>
            <span className="text-[10px]">/</span>
            <span className="text-text-primary font-extrabold">{candidate.full_name}</span>
          </div>
        ...
        <button
          ...
          className="silver-button-primary flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-wider active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* A. Candidate Profile */}
          <div className="silver-gradient-panel rounded-[26px] p-6">
            <h3 className="mb-5 text-[11px] font-extrabold font-mono uppercase tracking-widest text-amber-500/85">
              Candidate Profile
            </h3>
            
            <div className="grid grid-cols-1 gap-x-6 gap-y-4 text-xs sm:grid-cols-2">
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Full Name</span>
                <span className="text-right font-extrabold text-[13px] text-text-primary">{candidate.full_name}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Phone Number</span>
                <span className="font-mono text-right font-extrabold text-[12px] text-text-primary">{candidate.phone}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Alt Contact</span>
                <span className="font-mono text-right font-extrabold text-[12px] text-text-primary">{candidate.alternate_phone || '--'}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Date of Birth</span>
                <span className="font-mono text-right font-extrabold text-[12px] text-text-primary">{candidate.date_of_birth || '--'}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Father Name</span>
                <span className="text-right font-extrabold text-[13px] text-text-primary">{candidate.father_name || '--'}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Address</span>
                <span className="max-w-[200px] truncate text-right font-extrabold text-[12px] text-text-primary" title={candidate.address}>{candidate.address || '--'}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Pincode</span>
                <span className="font-mono text-right font-extrabold text-[12px] text-text-primary">{candidate.pincode || '--'}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Branch</span>
                <span className="text-right font-extrabold text-[13px] text-text-primary">{candidate.branch}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Course</span>
                <span className="text-right font-extrabold text-[13px] text-text-primary">{candidate.course}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Date of Joining</span>
                <span className="font-mono text-right font-extrabold text-[12px] text-text-primary">{getJoiningDate()}</span>
              </div>
              <div className="flex justify-between rounded-xl border border-slate-200 bg-white/55 px-3 py-2.5 sm:col-span-2">
                <span className="text-text-muted font-mono uppercase text-[11px] font-black">Current Status</span>
                <span className="text-right font-extrabold text-[13px] text-text-secondary">
          ...
          {/* B. Past Employment */}
          <div className="silver-gradient-panel rounded-[26px] p-6">
            <h3 className="mb-2 text-[11px] font-extrabold font-mono uppercase tracking-widest text-amber-500/85">
              Past Employment
            </h3>
          ...
          {/* C. Document Vault */}
          <div className="silver-gradient-panel rounded-[26px] p-6">
            <h3 className="mb-4 text-[11px] font-extrabold font-mono uppercase tracking-widest text-amber-500/85">
              Document Vault
            </h3>
          ...
          <div className="silver-gradient-panel flex items-center justify-between rounded-[26px] p-6">
            <div>
              <h3 className="mb-1 text-[11px] font-extrabold font-mono uppercase tracking-widest text-amber-500/85">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="silver-gradient-panel flex h-full min-h-[500px] flex-col justify-between rounded-[26px] p-6">
            ...
            <div className="space-y-5">
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="silver-button-primary flex items-center gap-0.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold active:scale-95"
                >
                  + ADD PAYMENT
                </button>
              </div>
              ...
              <div className="grid grid-cols-4 gap-1 border-b border-border-secondary pb-1 text-[9px] font-bold font-mono text-center">
                {[
                  ...
                  <button
                    ...
                    className={`pb-2 border-b transition-all tracking-wider ${
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4 overflow-y-auto">
          <form 
            ...
            className="silver-gradient-panel my-8 w-full max-w-5xl rounded-[30px] p-5 shadow-[0_30px_60px_-24px_rgba(15,23,42,0.35)] text-slate-800 fade-in md:p-7"
          >
            <div className="mb-5 flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-indigo-600">Candidate profile</p>
                <h3 className="mt-2 text-2xl font-black text-slate-900">{candidate.full_name}</h3>
                <p className="mt-1 text-xs font-semibold text-slate-500">Modify student details, placement data, and verification status.</p>
              </div>
              <button 
                type="button"
                onClick={() => setIsEditModalOpen(false)} 
                className="rounded-full border border-slate-200 bg-white/80 p-2 text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
              <div>
                ...
                <input type="text" required value={editName} onChange={e => setEditName(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Phone Number *</label>
                <input type="text" required value={editPhone} onChange={e => setEditPhone(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Email Address *</label>
                <input type="email" required value={editEmail} onChange={e => setEditEmail(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Alternate Contact</label>
                <input type="text" value={editAltPhone} onChange={e => setEditAltPhone(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Date of Birth</label>
                <input type="date" value={editDob} onChange={e => setEditDob(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Father Name</label>
                <input type="text" value={editFatherName} onChange={e => setEditFatherName(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div className="md:col-span-2">
                <label className="mb-2 block text-sm font-black text-slate-700">Address</label>
                <input type="text" value={editAddress} onChange={e => setEditAddress(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Pincode</label>
                <input type="text" value={editPincode} onChange={e => setEditPincode(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Branch</label>
                <input type="text" value={editBranch} onChange={e => setEditBranch(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Course Domain</label>
                <input type="text" value={editCourse} onChange={e => setEditCourse(e.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div className="flex items-end">
                <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 bg-white/80 px-3 py-3 text-sm font-black text-slate-700">
                  <input type="checkbox" checked={editWorking} onChange={e => setEditWorking(e.target.checked)} className="h-4 w-4 rounded text-indigo-600 focus:ring-indigo-500" />
                  <span>Currently Working</span>
                </label>
              </div>

              <div className="md:col-span-2 border-t border-slate-200 pt-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600">Corporate placement info</h4>
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Placement Company</label>
                <input type="text" value={editCompany} onChange={e => setEditCompany(e.target.value)} placeholder="e.g. Zoho Corporation" className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Designation</label>
                <input type="text" value={editDesignation} onChange={e => setEditDesignation(e.target.value)} placeholder="e.g. Software Associate" className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Annual CTC (INR)</label>
                <input type="number" value={editCtc} onChange={e => setEditCtc(e.target.value)} placeholder="e.g. 450000" className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-black text-slate-700">Placement Status</label>
                <select value={editStatus} onChange={e => setEditStatus(e.target.value as Candidate['placement_status'])} className="h-12 w-full rounded-xl border border-slate-200 bg-white/85 px-3 text-sm font-bold text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10">
            ...
            <div className="mt-6 flex justify-end gap-3 border-t border-slate-200 pt-4">
              <button 
                type="button" 
                onClick={() => setIsEditModalOpen(false)} 
                className="silver-button-secondary rounded-xl px-5 py-3 text-xs font-black uppercase tracking-wide"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                className="silver-button-primary rounded-xl px-5 py-3 text-xs font-black uppercase tracking-wide"
              >
                Save Changes
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          ...
          >
            <div className="flex justify-between items-center border-b border-border-secondary pb-3">
              <h3 className="font-bold text-lg">Record Payment: {activeFinTab}</h3>
              <button 
                type="button" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm p-4">
          ...
          >
            ...
            <div className="space-y-3 text-xs">
              ...
              <div 
                className="border border-dashed border-border-primary hover:border-accent-orange/45 rounded-xl p-6 text-center space-y-2 cursor-pointer transition-all bg-bg-secondary/20 relative"
