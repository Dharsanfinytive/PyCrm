    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 fade-in text-slate-800 sm:px-6 lg:px-8">
      <div className="silver-gradient-panel rounded-[28px] p-5 md:p-7">
        <div className="flex flex-col gap-2">
          <span className="w-fit rounded-full border border-[#d3a663]/40 bg-[linear-gradient(135deg,#f9efe1,#f1d9b3,#efc680)] px-3 py-1 text-[9px] font-black uppercase tracking-[0.22em] text-[#5c3c1f]">
            Finance & Audit
          </span>
          ...
          <p className="mt-1 text-sm font-bold leading-6 text-slate-500">
            Select from the 9 specified audit and financial breakdown modules below.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-stretch">
        ...
        {/* Left Sidebar Menu */}
        <div className="silver-card w-full shrink-0 self-start rounded-[24px] p-3 lg:w-[280px]">
          <span className="mb-3 block px-2.5 text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Report Selection</span>
            ...
            {reportPills.map(p => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl border px-3.5 py-2.75 text-left text-[12.5px] font-black transition-all duration-200 ${
                    activeReportTab === p.id
                      ? 'border-[#9c6c35] bg-[linear-gradient(135deg,#56391c,#9c6c35,#d69a48)] text-white shadow-[0_14px_26px_-18px_rgba(120,79,32,0.9)]'
                      : 'border-slate-200 bg-white/80 text-slate-700 hover:border-[#d3a663] hover:bg-[linear-gradient(135deg,#f9efe1,#f1d9b3,#efc680)] hover:text-[#5c3c1f]'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${activeReportTab === p.id ? 'bg-white/15 text-white' : 'bg-slate-100 text-slate-600'}`}>
                      <Icon className="h-4 w-4" />
                    </div>
                    <span className="leading-5">{p.label}</span>
                  </div>
                  {p.count !== undefined && p.count > 0 && (
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${
                      activeReportTab === p.id 
                        ? 'bg-white text-[#8b5a2b]'
        ...
        <div className="silver-card min-h-[420px] flex-1 rounded-[24px] p-4 md:p-5">
          {renderReportContent()}
        </div>
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
          <div className="silver-gradient-panel my-8 w-full max-w-lg rounded-[28px] p-5 shadow-[0_30px_60px_-20px_rgba(15,23,42,0.35)] fade-in md:p-6">

            {/* Header */}
            <div className="flex items-start justify-between border-b border-slate-200 pb-4">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#d3a663]/40 bg-[linear-gradient(135deg,#f9efe1,#f1d9b3,#efc680)] text-[#5c3c1f]">
                  <UserIcon className="h-5 w-5" />
                </div>
                <div>
                  <span className="rounded-md border border-slate-200 bg-white/80 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">
                    {selectedStudent.candidate_code}
                  </span>
                  {!isEditing ? (
                    <h4 className="mt-2 text-lg font-black text-slate-900">{selectedStudent.full_name}</h4>
                  ) : (
                    <input
                      type="text"
                      ...
                      className="mt-2 w-full rounded-lg border border-slate-200 bg-white/80 px-2.5 py-2 text-sm font-black text-slate-800 outline-none transition focus:border-[#d3a663] focus:ring-4 focus:ring-[#d3a663]/15"
                    />
                  )}
                </div>
              </div>
              <button onClick={closeStudentCard} className="rounded-full border border-slate-200 bg-white/80 p-1.5 text-slate-600 transition hover:border-[#d3a663] hover:text-[#8b5a2b]"><X className="h-5 w-5" /></button>
            </div>

            {/* Details grid — view mode vs edit mode */}
            {!isEditing ? (
              <div className="grid grid-cols-2 gap-x-4 gap-y-3 border-b border-slate-200 pb-4 pt-4 text-xs">
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Course / Domain</span>
                  <span className="mt-1 block font-bold text-slate-700">{selectedStudent.course}</span>
                </div>
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Batch & Branch</span>
                  <span className="mt-1 block font-bold text-slate-700">{selectedStudent.batch} ({selectedStudent.branch})</span>
                </div>
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Contact</span>
                  <span className="mt-1 block font-black text-slate-700">{selectedStudent.phone}</span>
                </div>
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Email</span>
                  <span className="mt-1 block truncate font-black text-slate-700">{selectedStudent.email}</span>
                </div>
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Team</span>
                  <span className="mt-1 block font-bold text-slate-700">{selectedStudent.team_id}</span>
                </div>
                <div>
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Placement Status</span>
                  <span className="mt-1 block font-bold text-slate-700">{selectedStudent.placement_status.replace('_', ' ')}</span>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3 border-b border-slate-200 pb-4 pt-4 text-xs">
                <label className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-white/80 px-2.5 py-2 text-sm font-bold text-slate-700 outline-none transition focus:border-[#d3a663] focus:ring-4 focus:ring-[#d3a663]/15"
                  />
                </label>
                <label className="space-y-1.5">
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Batch</span>
                  <input
                    type="text"
                ...
                <label className="space-y-1.5">
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Branch</span>
                  <input
                    type="text"
                ...
                <label className="space-y-1.5">
                  <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Phone</span>
                  <input
                    type="text"
                    value={editForm.phone ?? ''}
                    onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })}
                    className="w-full rounded-lg border border-slate-200 bg-white/80 px-2.5 py-2 text-sm font-black text-slate-700 outline-none transition focus:border-[#d3a663] focus:ring-4 focus:ring-[#d3a663]/15"
                  />
                </label>
            ...
            {!isEditing && selectedStudent.placement_status === 'APPROVED' && (
              <div className="grid grid-cols-3 gap-2 pt-4 text-center">
                <div className="rounded-xl border border-slate-200 bg-white/60 p-2.5">
                  <div className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Payable</div>
                  <div className="mt-1 text-sm font-black text-slate-800">₹{selectedStudent.amount_payable.toLocaleString()}</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white/60 p-2.5">
                  <div className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Paid</div>
                  <div className="mt-1 text-sm font-black text-emerald-600">₹{selectedStudent.total_paid.toLocaleString()}</div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white/60 p-2.5">
                  <div className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Pending</div>
                  <div className="mt-1 text-sm font-black text-rose-600">₹{selectedStudent.pending_amount.toLocaleString()}</div>
                </div>
              </div>
            )}
              ...
              {!isEditing ? (
                <>
                  <button
                    ...
                    className="silver-button-secondary rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.16em]"
                  >
                    Close
                  </button>
                  <button
                    onClick={startEditStudent}
                    className="silver-button-primary rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.16em]"
                  >
                    <span className="flex items-center gap-2"><Pencil className="h-3.5 w-3.5" /> Edit Details</span>
                  </button>
                </>
              ) : (
                <>
